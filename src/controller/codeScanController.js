...
// 4.
    ...
    console.log("get is Scanned",getTable[0].isscanned);    
    if(getTable[0].isscanned){
      console.log("inValid allReady scanned");
      res.status(400).json({message:" inValid allReady scanned"});
      return;
    }
    else{
      const Scanned = await prisma.$queryRawUnsafe(`UPDATE ${tableName} SET isscanned = TRUE WHERE unique_code = $1`, uniqueCode )
